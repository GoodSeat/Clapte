...
namespace GoodSeat.Sio
{
	...
	public class Reflector<T> where T : class
	{
		...
		/// <param name="folderName">可変数の取得対象アプリケーションフォルダ名。</param>
		/// <returns>取得されたレフレクタ情報。取得に失敗した場合、null。</returns>
		...
		/// <summary>
		/// 実行可能ファイルと同ディレクトリ内で、取得可能な指定の型のリフレクタ情報リストを返します。
		/// </summary>
		/// <typeparam name="T">対象の型。</typeparam>
		/// <returns>リフレクタ情報リスト。</returns>
		public static Reflector<T>[] FindReflectors() { return FindReflectors(""); }
        ...
        protected static bool HasCorrectInterface(Type t, params string[] iNames)
		{
			...
			{
				if (t.GetInterface(name) == null) return false;
			}
		...
		/// </summary>
		/// <param name="t">検証する型。</param>
		/// <param name="types">継承検証する可変数の型。</param>
		/// <returns>typesで指定された型のすべてを継承しているか否か。</returns>
		protected static bool HasCorrectInterface(Type t, params Type[] types)
		{
			foreach (Type type in types)
			{
				if (!t.IsSubclassOf(type)) return false;
			}
			return true;
		}
		...
		/// リフレクション情報を初期化します。
		/// </summary>
		/// <param name="type">対象とするクラスの型。</param>
		...
		/// <summary>
		/// 対象とするクラスのインスタンスを作成します
		/// </summary>
		/// <param name="args">コンストラクタの引数。</param>
		/// <returns>初期化された対象クラスのインスタンス。初期化に失敗した場合、nullを返します。</returns>
