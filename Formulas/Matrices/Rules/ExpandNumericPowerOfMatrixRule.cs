...
namespace Liffom.Formulas.Matrices.Rules
{
	...
	/// </summary>
	public class ExpandNumericPowerOfMatrixRule : Rule
	{
		static ExpandNumericPowerOfMatrixRule s_entity;

		/// <summary>
		/// 整数乗の累乗展開を規定するルールの実体を取得します。
		/// このルールにはプロパティが存在せず、再帰呼び出しにおいても結果は不変のため、通常この静的プロパティを使用することが推奨されます。
		/// </summary>
		public static ExpandNumericPowerOfMatrixRule Entity
		{
			get
			{
				if (s_entity == null) s_entity = new ExpandNumericPowerOfMatrixRule();
				return s_entity;
			}
		}
		...
		{
			var power = target as PowerOfMatrix;
			if (power == null) return false;

			if (!(power.Base is OperatorMultiple) && !(power.Base is Matrix)) return false;
		...
		protected override Formula OnTryMatchRule(Formula target)
		{
			...
			return new ProductOfMatrix(sectionList.ToArray());
		}

		protected override IEnumerable<Type> OnGetPreDemandRules()
		{
			yield return typeof(FactorOutMinusOneOfExponentOfMatrixRule); // 負数による累乗では、^-1を括り出し済み
		}

		protected override IEnumerable<Type> OnGetPostDemandRules()
		{
			yield return typeof(CombineProductOfMatrixToPowerRule); // 逆変換の整理傾向
		}

		protected override IEnumerable<Rule> OnGetReverseRule()
		{
			yield return new CombineProductOfMatrixToPowerRule(); // a.a → a^^2
		}
