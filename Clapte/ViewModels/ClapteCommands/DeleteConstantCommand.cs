...
{
    /// <summary>
    /// ユーザー定義定数の削除コマンドを表します。
    /// </summary>
    public class DeleteConstantCommand : ClapteCommand
    {
        /// <summary>
        ...
        static DeleteConstantCommand()
        {
            DefineRegex = new Regex(@"^\s*delete\s*\-\>\s*(?<name>\S+)");
        }
        ...
        public DeleteConstantCommand(ClapteCoreViewModel owner)
            : base(owner) 
        {
        }

        /// <summary>
        /// 削除候補となるユーザー定義定数の定義オブジェクトを設定もしくは取得します。
        /// </summary>
        private ConstantDefine Target { get; set; }

        /// <summary>
        /// 削除対象となるユーザー定義定数リストを取得します。
        /// </summary>
        ...
        public override ClapteCommandResult TryInitializeCommand(string text)
        {
            ...
            var message = string.Format("操作の続行により、ユーザー定義定数 {0} を削除します。", name);
            return new ClapteCommandResult("ユーザー定義定数の削除", message, ToolTipIcon.Warning, true);
        }
        ...
        public override ClapteCommandResult DoAction()
        {
            ...
            string title = "ユーザー定義定数の削除";
            string message = string.Format("ユーザー定義定数 {0} を削除しました。", Target.Name);
            Target = null;
