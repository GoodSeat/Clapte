...
namespace GoodSeat.Liffom.Processes
{
	...
	public class BrentMethod : SolveEquation
	{
		...
		{
			return GetSolution(target, about, LowerLimit, UpperLimit, ErrorTolerance, MaxTryCount);
		}
		...
		/// <param name="lowerLimit">解の下限値</param>
		/// <param name="upperLimit">解の上限値</param>
		/// <param name="error">許容誤差</param>
		/// <param name="maxTryCount">最大試行回数</param>
		/// <returns>求められた解。ただし、最大試行回数まで解が見つからなかった場合、null。</returns>
		public static Equal GetSolution(Equal target, Variable x, Numeric lowerLimit, Numeric upperLimit, double error, int maxTryCount)
		{
			...
			// それなりに区間を探す
			SearchLimit(f, x, ref upperLimit, ref lowerLimit);

			Numeric solution = GetSolution(f, x, lowerLimit, upperLimit, error, maxTryCount);
#if DEBUG
		...
		public static Numeric GetSolution(Formula f, Variable x, Numeric lowerLimit, Numeric upperLimit, double error, int maxTryCount)
		{
			...
			for (int i = 0; i < maxTryCount; i++)
			{
				...
				Real tol1 = (new Numeric(0.5 * error)).Data;
				Real xm = new ValidReal(0.5 * (c - b));
				...
				if (Math.Abs(e) >= tol1 && Math.Abs(fa) > Math.Abs(fb))
				{
					...
					if (a == c)
					{
						p = new ValidReal(2d * xm * s);
						q = new ValidReal(1d - s);
					}
					else
					{
						q = fa / fc;
						var r = fb / fc;
						p = new ValidReal(s * (2 * xm * q * (q - r) - (b - a) * (r - 1)));
						q = new ValidReal((q - 1) * (r - 1) * (s - 1));
					}
					if (p > 0) q = -q; // 区間内かどうかチェック

					p = new ValidReal(Math.Abs(p));
					Real min1 = new ValidReal(3 * xm * q - Math.Abs(tol1 * q));
					Real min2 = new ValidReal(Math.Abs(e * q));
		...
		/// <param name="lowerLimit">解の探索下限値</param>
		/// <param name="upperLimit">解の探索上限値</param>
		private static void SearchLimit(Formula f, Variable x, ref Numeric upperLimit, ref Numeric lowerLimit)
		{
			var token = new DeformToken(Formula.SimplifyToken, Formula.CalculateToken, Formula.NumerateToken);
