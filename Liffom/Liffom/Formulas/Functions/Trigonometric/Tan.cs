...
namespace GoodSeat.Liffom.Formulas.Functions
{
    ...
    public class Tan : TrigonometricFunction
    {
        ...
        public Tan(Formula f) : base(f) { Argument = new Argument(f); }
        ...
        {
            if (arg is Numeric) return new Numeric((arg as Numeric).Figure.Tan());
            ...
            var e = Napiers.e;
            var i = Imaginary.i;
            return ((e ^ (arg * i)) - (e ^ (-arg * i))) / (i * ((e ^ (arg * i)) + (e ^ (-arg * i))));
        }
        ...
        public override Formula GetRationalValueOf(TriRad triRad)
        {
            switch (triRad)
            {
                ...
                case TriRad.Rad270:
                case TriRad.Rad90: return new Numeric(double.PositiveInfinity);

                case TriRad.Rad30:
                case TriRad.Rad210: return Formula.Parse("1/3^(1/2)");
                case TriRad.Rad45:
                case TriRad.Rad225: return Formula.Parse("1");
                case TriRad.Rad60:
                case TriRad.Rad240: return Formula.Parse("3^(1/2)");

                case TriRad.Rad120:
                case TriRad.Rad300: return Formula.Parse("-3^(1/2)");
                case TriRad.Rad135:
                case TriRad.Rad315: return Formula.Parse("-1");
                case TriRad.Rad150:
                case TriRad.Rad330: return Formula.Parse("-1/3^(1/2)");
        ...
        public override IEnumerable<Rule> GetRelatedRulesOf(DeformToken deformToken, Formula sender, DeformHistory history) 
        {
            ...
            if ((deformToken.Has<CalculateToken>() || deformToken.Has<DifferentiateToken>()) && sender is Differentiate)
            {
                ...
                yield return new InstantPatternRule(new Differentiate(new Tan(x), x), 1 / (new Cos(x)) ^ 2);
